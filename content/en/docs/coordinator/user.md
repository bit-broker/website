...
New users will be present in the system right away. They can immediately take part as data consumers by being [assigned policy-based, consumer keys](/docs/api-principles/authorisation/#obtaining-a-consumer-key). However, [a further step](#promoting-a-user-to-coordinator) is required to promote them to have coordinator status.
...
The body of this response will contain the coordinator key, which the newly promoted user should utilise [to authorise](/docs/api-principles/authorisation/) their own calls to the Coordinator API. For example:
