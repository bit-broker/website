...
Regardless of the type of installation you are using, you should be aware of the ways in which fresh installations perform certain bookstrap operations.
...
As we will explain below, this user is linked to the [authorisation](/docs/api-principles/authorisation/) of API calls. Since this authorisation is only applied in Kubernetes installs, the boostrap user is only of significance there. If you are installing using [Docker Compose](#docker-compose-local-installation) or on your [bare machine](#direct-installation-on-a-physical-machine), it is not a vital concept.
...
This is not a concern, if you are installing using [Docker Compose](#docker-compose-local-installation) or on your [bare machine](#direct-installation-on-a-physical-machine).
...
The Kubernetes install steps below, will contain more information about how to access and use this bootstrap token.
...
{{% alert color="info" %}}
This type of deployment will give you full access to the complete BitBroker feature set, including full key and policy enforcement. The system will be accessible to anyone to whom you grant access.
{{% /alert %}}
...
{{% alert color="warning" %}}
This type of deployment will give you full access to the complete BitBroker feature set, including full key and policy enforcement. But the system will only be available on your local machine. This type of install is suitable for _development purposes_ only.
{{% /alert %}}
...
{{% alert color="warning" %}}
Before starting, make sure that Kubernetes is enabled within Docker Desktop. Settings » Kubernetes » Enable Kubernetes.xxxx
{{% /alert %}}

{{% alert color="warning" %}}
Before starting, make sure that you don't have any other service listening on `port 80` of your `localhost`.xxxx
{{% /alert %}}
...
This will have created a'bit-broker' directory and you should move into there:
...
This `unpack` step, makes sure that all the dependant node packages needs to operate all aspects of the system are downloaded and ready.
...
The simple delete the Git cloned folder you created earlier.
